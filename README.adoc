...
ℹ️ All keys are converted to symbols before being processed. This is done to ensure consistency and improve debuggablity when dealing with raw input that might be a mix of strings and/or symbols.
...
Loaders are a great way to load _default_ configuration information for your application which can be in multiple formats. Loaders can eitehr be defined when creating a new registry instance or added after the fact. Here are a few examples:
